...
log using "${logfiles}\URE_investigation", text replace
...
by familie_id: egen yrs = count(famforbrug1)
gen est_sample = alder_head_08 >= 30 & alder_head_08 <= 55 & yrs >= 5


bys familie_id: egen mean_inc_after_tax = mean(famindkefterskat/PRISS112*100) if year>=2009
by familie_id: egen mean_cons_with_interest = mean((famforbrug1+ famrentudgpr)/PRISS112*100) if year>=2009
...
by familie_id: egen mean_maturing_assets = mean((famoblakt/5 +   fampantakt/5 + fambankakt)/PRISS112*100) if year>=2009
...
by familie_id: egen mean_maturing_liabiliies = mean((fambankgaeld + mortgage_refinance_1y +(famqpassivn -fambankgaeld -famoblgaeld -fampantgaeld )/5)/PRISS112*100) if year>=2009
gen URE = mean_inc_after_tax - mean_cons_with_interest + mean_maturing_assets - mean_maturing_liabiliies

by familie_id: egen mean_nominal_assets = mean((famoblakt + fampantakt + fambankakt)/PRISS112*100) if year>=2009
by familie_id: egen mean_nominal_liabilities = mean(famqpassivn/PRISS112*100) if year>=2009
gen NNP = mean_nominal_assets-mean_nominal_liabilities

sum URE NNP if est_sample
