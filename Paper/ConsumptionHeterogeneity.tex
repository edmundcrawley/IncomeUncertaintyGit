...
\begin{document}\bibliographystyle{\econtexBibStyle}
	\bibliographystyleappendix{\econtexBibStyle}
	\input Switches.tex
	...
	\begin{abstract}
		...
		We document heterogeneity in the marginal propensity to consume (MPC) across a variety of household characteristics relevant to understanding heterogeneous agent models and monetary policy transmission. We find a strong relationship between household liquid wealth and MPC out of both transitory and permanent income shocks. Households in the lowest liquid wealth quintile live near hand-to-mouth. Households in the top liquid wealth quintile have an MPC of 0.32 out of transitory income shocks and 0.53 out of permanent income shocks. We show that household liquid wealth is able to predict the MPC for households to within a few percentage points for every other household characteristic we look at. These results come from a new empirical method that overcomes the time aggregation bias found in existing literature, along with administrative data from Denmark that allows us to finely divide the population to identify heterogeneous behavior. We use our results to analyze monetary policy transmission mechanisms in both Denmark and the U.S..
	...
	This paper adds to the existing literature on estimating household consumption behavior in response to income shocks. Much of the older literature focused on estimating an average MPC for the economy as a whole, while recently there has been an effort to identify heterogeneity in consumption behavior. Table \ref{table:MPCLiterature} shows a selection of the population average estimates from the literature. An overview of the older literature on consumption responses to income changes is found in \cite{jappelli_consumption_2010}. The empirical methods fall into three categories.
	...
	The first category---our method falls in this category---is to impose covariance restrictions on panel data of income and consumption and use these to identify the consumption response to income shocks of differing persistence. This method has the advantage that it uses `typical' income shocks for identification, as opposed to some unusual income shock such as a lottery win or a stimulus check. However, it is potentially subject to model misspecification. The most well-known paper to use this method is by \cite{blundell_consumption_2008} (hencforth BPP), which uses imputed non-durable consumption data based on food expenditure reported in PSID data. BPP estimates a consumption elasticity (closely related to an MPC if households' consumption level is close to their income) and find almost no consumption response to transitory shocks. Both \cite{crawley_time_2020} and \cite{commault_how_2017} suggest misspecification problems with the methodology in BPP that bias the consumption response to transitory shocks down: \cite{crawley_time_2020} with respect to time aggregation and \cite{commault_how_2017} with respect to income shock persistence combined with transitory consumption dynamics. In this paper, we account for both of these problems. Our methodology differs from that in \cite{commault_how_2017} in that we apply a slightly stricter condition on transitory consumption dynamics and a slighter looser conditions on transitory income dynamics. We compare the results from our methodology and that in \cite{commault_how_2017} in section ********************** and find they are substantially in agreement. The BPP method, and those closely related to it, has become a standard tool in the consumption literature. See, for example, \cite{violante_wealthy_2014}; \cite{auclert_monetary_2017}; and \cite{manovskii_how_2017}.
	
	The second category of methods to estimate consumption behavior is to identify a natural experiment and measure the consumption response to it. For example, \cite{johnson_household_2006} use randomly assigned timing of 2001 tax rebates and questions in the U.S. Consumer Expenditure Survey to identify a three-month aggregate marginal propensity to consume of 0.2 to 0.4. Identification in this second category is often the more clear cut, but MPC estimates vary and there is no strong consensus. Moreover, some identification issues arise as to when exactly households learn about the payment versus when it is received, and the extent to which external validity extends from these natural experiments to the kinds of `typical' transitory shocks found in heterogeneous agent models is unclear. Some of these studies find a smaller MPC for positive shocks than negative shocks---for example, \cite{bunn_consumption_2018}. In this paper we implicitly assume that the response is symmetric. In reality, our estimates represent an average of positive and negative shock reactions.
	
	Finally, the third category involves simply to asking individuals how much of a transitory income change they would consume. \cite{jappelli_fiscal_2014} find an aggregate MPC of 0.48 using the Italian Survey of Household Income and Wealth and are able to identify clear differences across levels of liquid wealth. \cite{fuster_what_2018} find a lower aggregate MPC in the NY Fed's Survey of Consumer Expectations, but they find heterogeneity by both size and sign of the shock. Of course the reliability of these studies is limited by the accuracy of households' own response to the question.
	
	Big data sources, such as credit and debit card transaction data and registry data of the type used in this paper, have recently allowed researchers to dig into how consumption behavior varies systematically across household characteristics. \cite{fagereng_mpc_2016} finds a similar, albeit weaker, relationship between household liquid wealth and MPC using lottery winnings and Norwegian registry data and income and wealth. Credit and debit card transactions from banks or financial aggregators can be used to identify heterogeneous consumption responses at a high frequency. \cite{baker_debt_2015} is one of the first papers to make use of these type of data and looks at how debt consumption behavior varies with household debt. Some more recent papers have focused on the relationship between liquidity and consumption smoothing: \cite{gelman_what_2016} finds large differences in the impulse response to a tax rebate at a monthly frequency across household quintiles of cash-on-hand; \cite{ganong_wealth_2020} exploit a rich source of data from JPMorgan and, similar to our paper, finds differences in household liquid wealth can explain differences in consumption elasticity across a variety of other dimensions, with a focus on race. At a very high frequency, \cite{vardardottir_liquid_2016} looks at consumption around payday in Icelandic data and finds strong responses even for households with liquid assets. Big, high frequency, spending data has also shown its value during the pandemic. \cite{baker_income_2020}, \cite{chetty_economic_2020}, and \cite{karger_heterogeneity_2020} all point to high MPC in the days and weeks following the first round of stimulus checks, especially for low-income or low liquid wealth households. 

	The method and the MPC estimates obtained in this paper can be used to discipline the growing literature on heterogeneous agent models. It is now widely recognized that the way in which macroeconomic shocks propagate can hinge not only on the average household MPC for an economy, but also on wealth transfers between households with different MPCs. In this paper, we provide new estimates for the sufficient statistics suggested in \cite{auclert_monetary_2017} to measure the size of monetary policy transmission mechanisms. We believe these sufficient statistics to be highly informative of the mechanisms behind monetary policy, but the method calls for high quality MPC estimates across the relevant dimensions of heterogeneity that have previously been hard to come by. \cite{ampudia_monetary_2018} also use Auclert's framework, in this case to look at differences in monetary policy transmission between European countries, but this paper does not attempt to estimate MPCs. \cite{patterson_2019} provides estimates for how much aggregate income fluctuations are amplified by MPC heterogeneity, using data from the PSID. \cite{wong_population_2016} also takes an empirical approach to identifying how the consumption response to monetary policy shocks varies with age.
...
*****************\\
	...
	\begin{figure}
		\centering
		\includegraphics[scale=0.4]{\figdir/VarianceByLiquidWealth.pdf}
		\centering
		\includegraphics[scale=0.4]{\figdir/MPXByLiquidWealth.pdf}
	...
	The two black lines show the regression coefficients where the sample is restricted to households in the lowest and highest quintiles of liquid wealth (averaged over the observed period), respectively. For households in the lowest quintile there is little evidence of consumption smoothing: the consumption response to income growth over one year is both high and almost identical to that over 7 years, strongly suggesting the MPCs for this group out of transitory and permanent shocks are similar and high. Indeed, this is what we see in the lowest quintile of figure \ref{fig:MPXByLiquidWealth}---a result that is highly robust to misspecification. Households in the top quintile of liquid wealth show a clear upward slope in figure \ref{fig:GrowthReg}, indicating a substantial degree of consumption smoothing. The fact that the regression coefficient for this group appears to asymptote well below 1 also suggests, in contrast to standard buffer-stock models, that the MPC out of permanent shocks for liquid households is significantly lower than 1.
	
	\subsection{Comparison with BPP and \cite{commault_how_2017}} \label{result_comparison}
	\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.6]{\figdir/Commault_BPP.pdf}
		\caption{Estimation using methods from \cite{blundell_consumption_2008} and \cite{commault_how_2017}}
		\label{fig:compare}
	\end{centering}
	\end{figure}
	Figure \ref{fig:compare} shows the transitory MPX estimates from the Danish data, using the estimation methodology from BPP and \cite{commault_how_2017}. The methodology from BPP is subject to two criticisms. The first criticism, detailed in \cite{crawley_time_2020}, is that the time aggregated nature of the income data can result in a large downward bias in the transitory MPX estimate. The intuition, although there is more nuance, is that the BPP method uses the change in income from period $t$ to period $t+1$ as an instrument for transitory shocks in period $t$, under the assumption that the permanent shocks are uncorrelated from period to period. Therefore, a negative change in income is related to a positive transitory shock in the previous period but unrelated to permanent shocks in the previous period. This does not hold when permanent shocks can occur, for example, half way through the period. In this case, the permanent component on income will display positive correlation across periods---see \cite{working_note_1960}---and so a negative change in income is also associated with a negative permanent income shock in the previous period. As a negative permanent shock will likely result in lower consumption, negative permanent shocks---that are mistaken for positive transitory shocks---pull the estimated consumption response to transitory shocks in the BPP method toward zero.
	
	The second criticism comes from \cite{commault_how_2017} who shows that the BPP methodology is not robust when transitory income shocks have some persistence and consumption does not follow a random walk. The suggested robust estimator uses the change in income from period $t+1$ to $t+2$ as an instrument for transitory shocks in period $t$. For this estimate to be unbiased, transitory shocks to income must have some persistence up to one year, but not further than that. Under certain conditions, this robust estimator is also free of time-aggregation bias.
	
	The methodology in this paper is also robust to both criticisms, although we have taken a different approach to \cite{commault_how_2017}. In place of the assumption that transitory income persists less than one year, we allow for two years of persistence but also make the assumption that the consumption response to the transitory shocks lasts no more than two years.
	
	The estimation results for the transitory MPX for all three methods is shown in figure \ref{fig:compare}. Both our baseline estimates and those form the Commault robust estimator show a clear negative relation between MPX and liquid wealth quintile, while the BPP methodology estimates do not. The Commault robust estimator has larger standard errors than our baseline and also tends to find lower MPX than our baseline, possibly as result of transitory income persistence longer than one year. In online appendix ************ we describe in more detail the differences between these methodologies. 
	...
	\begin{figure}
		\centering
		\includegraphics[scale=0.4]{\figdir/VarianceByNetWealth.pdf}
		\centering
		\includegraphics[scale=0.4]{\figdir/MPXByNetWealth.pdf}
	...
	\begin{figure} 
		\begin{centering}
			\includegraphics[scale=0.85]{\figdir/MPXByUREdetailsPaper.pdf} \\
			\includegraphics[scale=0.4]{\figdir/MPXByNNP.pdf}
			\includegraphics[scale=0.4]{\figdir/MPXByIncome.pdf}
	...
	\begin{figure}
		\centering
		\includegraphics[scale=0.4]{\figdir/NW_predict_from_HHCHaracteristics.pdf}
		\centering
		\includegraphics[scale=0.4]{\figdir/URE_predict_from_HHCHaracteristics.pdf}
	...
	\begin{center}
		\begin{table}
			...
			\begin{center}
				\input \tabdir/prediction_errors_with_income.tex
			\end{center}
		\end{table}
	\end{center}

	Indeed, conditional on liquid wealth quintile, household income quintile has very little, if any, further information on a household's MPX. This can be seen in the step pattern of figure, in which MPX steps down along the liquid wealth quintile, but if flat along the income quintile.
	
	\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.3]{\figdir/TranMPXByIncLiq.png}
		\caption{Transitory MPX estimation by Liquid Wealth and Income Quintile}
		\label{fig:LW_Income}
	\end{centering}
	\end{figure}
	...
	**************NEED TO WRITE THIS UP****************
	\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.4]{\figdir/pri_nonpri_tranVar_liquidwealth.pdf}
		\includegraphics[scale=0.4]{\figdir/pri_nonpri_tranMPX_liquidwealth.pdf}
		\caption{Comparing private and non-private sector workers}
...
\end{figure}
	...
	Our data have two significant advantages over previous efforts. First, our sample is large, containing most households in Denmark. Second, we have detailed balance sheet information available.
	...
	\subsection{Results}
	Our estimates of the sufficient statistics for Denmark and the U.S. are shown in table \ref{table:suff_stats}. The aggregate income channel is summarized by $\mathcal{M}$ that we estimate to be 0.55 in Denmark and 0.45 in the U.S.. This means that if income for all households in the economy increased by 1\%, aggregate consumption growth would increase by 55 basis points (45 in the U.S.), broadly in line with calibrations of saver-spender models designed to fit evidence from \cite{campbell_consumption_1989}. The lower number in the U.S. results from greater income inequality than in Denmark, and the fact that high income households (who receive most of a proportional income increase) have relatively lower MPXs.
	...
	We also see large differences between Denmark and the U.S. in the Fisher and interest rate exposure channel, both of which are very large for Denmark but less important for the U.S.. $\mathcal{E}_P$ is estimated to be  negative 0.80 in Denmark, suggesting that a one-time increase in the price level of 1\% increases aggregate consumption growth by 80 basis points due to redistribution from those with large nominal assets to those with large nominal debts. This Fisher channel of monetary policy is emphasized in \cite{doepke_inflation_2006}. The fact that household debt in the U.S. is less than half that of Denmark contributes to the far smaller estimate of negative 0.25 for the U.S.. The interest rate exposure channel is also large in Denmark, while almost non-existent in the U.S.. We estimate  $\mathcal{E}_R$ to be  negative 0.29 in Denmark, suggesting that a 1\% increase in the interest rate decreases aggregate household expenditure growth by 26 basis points. The lower prevalence of variable rate mortgages in the U.S. compared to Denmark contributes to a small interest rate exposure channel there. However, this does not exclude the possibility that such a channel could operate through refinancing and long-term interest rates, as highlighted in \cite{eichenbaum_2018}.
	\begin{center}
		\begin{table}
			...
			\begin{center}
				\input \tabdir/sufficient_stats_with_US_se.tex
			\end{center}
		\end{table}
	\end{center}
	...
	An obvious question is why we have chosen not to use the well-known methodology of \citeappendix{blundell_consumption_2008}, who achieve identification of transitory shocks from the facts that: (i) transitory income shocks in period $t$ will mean-revert in period $t+1$; and (ii) permanent shocks in period $t$ are uncorrelated with income changes in period $t+1$.\footnote{\citeappendix{kaplan_how_2010} show in discrete time simulations that the methodology works reasonably well for standard calibrations of buffer-stock models and end up concluding, ``The BPP insurance coefficients should become central in quantitative macroeconomics.'' However, some recent papers such as \citeappendix{commault_how_2017} and \citeappendix{hryshko_income_2018} have pointed to other potential problems of the methodology.} Unfortunately, as noted in \citeappendix{working_note_1960}, in time aggregated data (ii) does not hold---a time aggregated random walk is positively autocorrelated. 
	While macroeconomists have long been aware of the importance of time aggregation in time series regressions (see \citeappendix{campbell_consumption_1989} for a well-known example), the problem has been overlooked by the household finance and labor economics literature.\footnote{For examples, see \citeappendix{moffitt_trends_2012}; \citeappendix{meghir_income_2004}; \citeappendix{nielsen_impact_2004}; \citeappendix{heathcote_unequal_2010}; and more recent quantile regression approaches such as \citeappendix{arellano_earnings_2017}.} We will therefore briefly describe the problem here. For a more detailed account with particular attention to BPP, see \citeappendix{crawley_time_2020}.
	...
	The observable income is the sum of all the income that was received by a household during the year. The key insight of \citeappendix{working_note_1960} is that even if there is no correlation between changes in income at the underlying frequency (it is a random walk), changes in the resulting time aggregated series will show positive autocorrelation.
	...
	While it would be possible to stick very closely to the original BPP model and adjust the covariance restrictions to take account of the time aggregation problem,\footnote{\citeappendix{crawley_time_2020} takes this more straightforward approach using the same PSID data as used in BPP.} we have found that in practice the underlying assumptions made by BPP (in particular that consumption follows a random walk) do not fit with the data. The random walk assumption was previously thought to be benign. Not only were the estimates of the consumption response to transitory shocks in BPP small and consistent with such an assumption, \citeappendix{kaplan_how_2010} show that without time aggregation, the BPP method correctly identifies the transitory consumption response in the period of the income shock regardless of the consumption dynamics going forward. This fact is again not robust to the time aggregation problem. With time aggregation taken into account, the estimates are highly sensitive to assumptions about short-term consumption dynamics---online appendix \ref{BPP_compare} shows how the random walk assumption affects MPX estimates. Therefore we have chosen to attain identification in a manner similar to \citeappendix{carroll_nature_1997}, which allows us to be agnostic about the exact short-term dynamics of income and consumption.
	
	\section{Comparison to \cite{blundell_consumption_2008}} \label{BPP_compare}
	In this section we provide MPX estimates using the Danish data derived from the original estimation method from BPP.\footnote{As in our baseline, we perform the estimation in levels, not logs, so that we can interpret our results as MPX provided the variance of permanent income change over the sample period is not large.} The estimates are shown in figure \ref{fig:BPP_liquid}. Two different sets of questions arise: 1) why do these results from the Danish data look so different to those obtained from the PSID, and 2) why do these results look so different to the baseline results presented in the main text?
	\begin{figure} 
	\begin{centering}
	...
	\end{centering}
	\end{figure}
	\subsection{Why do these results from the Danish data look so different to those obtained from the PSID?}
	\begin{itemize}
		\item Timing of consumption - PSID consumption is snapshot at year end, Danish data is time-aggregated.Note: our method DOES NOT WORK when there is a snapshot of consumption at the end of the year---we would need to assume much more about the path of consumption. (Commault may get away with it because she identifies fairly persistent transitory shocks so the year-end effect may be similar to the intial impact.)
		\item Durables - PSID is non durable only, our data includes durables (see below for discussion)
		\item PSID consumption is MUCH less tied to income than in the Danish data - our prior is that this is a problem with the PSID. See links.
	\end{itemize}
	\subsection{Why do these results look so different to the baseline results presented in the main text?}
	\begin{itemize}
		\item Bring in appendix I started about BPP, random walks and durables.
		\item Note: Random walk assumption is not-viable, make note of estimate recovered if we keep this assumption.
	\end{itemize}


	\section{Comparison to \cite{commault_how_2017}}
	\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.6]{\figdir/Commault.pdf} 
		\caption{Commault Robust Estimator for t=1 and 2}
		\label{fig:commault}
	\end{centering}
	\end{figure}
	\begin{itemize}
	\item Commault's robust estimator is roughly in line with our estimates, though lower.
	\item Commault assumes transitory income lasts no more that one year, or alternatively two years. The graph shows estimates for both the one and two year assumptions.
	\item There is clear evidence that the two year estimates are statistically significant (check if relevant covaraince is statistically significant)). This would bias her estimates down, just as she shows in the paper that persistence to one year biases BPP down.
	\item Overall the jury is out, but I would bet on our method
	\item Note the intuition graph strongly suggests our results are more reasonable.
	\end{itemize}
	...
	\begin{figure} 
		\begin{centering}
			\includegraphics[scale=0.6]{\figdir/MPXByAge.pdf} 
			\caption{MPX by Age}
			\label{fig:MPXByAge}
		\end{centering}
	\end{figure}
	...
	\begin{center}
		\begin{table}
			...
			\label{table:aggElas}
			\input \tabdir/URE_NNP_table.tex
		\end{table}
	\end{center}
	Much of the URE and NNP exposure is not held directly on the balance sheet of households but instead indirectly through pension funds, corporates, and the government. There is significant evidence that the MPX out of shocks to the value of pension wealth, stocks, or the government balance sheet is substantially lower than the MPX from income. We choose to use the estimate from \citeappendix{maggio_stock_2018} that households' MPX from changes in stock market wealth is about 10\%. This choice is the most quantitatively important for the sufficient statistics, as the bin containing the most exposure is the financial sector, which is positively exposed to interest rate increases. This positive interest rate exposure may seem surprising because banks are typically thought to have long-term assets and short-term debt that would result in negative URE exposure. However, our findings are in line with those of \citeappendix{landier_banks_2013}, who find that the financial sector benefits from interest rate hikes overall. An important caveat is due here: we focus on the MPX out of changes in the assets indirectly held by households through the financial sector and do not assume any spending or lending response at the bank level. This assumption may be reasonable in good times when banks are not credit constrained, but may not hold during a banking crisis. Financial frictions could possibly result in monetary policy being much less effective during a banking crisis as the interest rate exposure channel to household spending is counterbalanced by a channel from bank balance sheet interest rate exposure to lending.\footnote{It should be noted that our analysis is all on the household side. Evidence suggests that firms are also sensitive to changes in cash flow; for example, see \citeappendix{blanchard_what_1994}.} 
	...
	Empirical evidence suggests that in fact the consumption response to a transitory shock decays quickly in the first few months and then more slowly after that.\footnote{Both \citeappendix{fagereng_mpc_2016} and \citeappendix{gelman_what_2016} provide evidence for this.} The ``Fagereng et al.'' line in figure \ref{fig:DecayBias} shows the MPC estimate in simulated data in which the consumption response decays according to the estimates made in \citeappendix{fagereng_mpc_2016}. In this case, the fast decay in the first few months results in a smaller bias than the exponential case for low MPCs, while the fact that the decay is slower following these first months results in a larger bias for high MPCs. Overall it seems likely that our assumption about the persistence of the consumption response leads to a slight downward bias across the range of MPCs.
	...
	\end{align*}
	In \citeappendix{fagereng_mpc_2016} the T year MPC is estimated as a function:
	\begin{align*}
	...
	\begin{center}
		...
		\input \tabdir/Psi_array_empirical.tex
	\end{center}
	...
	\begin{figure} 
		\begin{centering}
			\includegraphics[scale=0.4]{\figdir/IncreasingDiff.pdf}
	...
	\begin{figure} 
		\begin{centering}
			\includegraphics[scale=0.4]{\figdir/IncomeGrowthStd.pdf}
...
\end{itemize}
	...
	\begin{figure}
		\begin{centering}
			\includegraphics[scale=0.4]{\figdir/Robust_tranMPX_liquidwealth.pdf}
			\includegraphics[scale=0.4]{\figdir/Robust_tranMPX_URE.pdf}
	...
	\begin{figure} 
		\begin{centering}
			\includegraphics[scale=0.4]{\figdir/Logs_tranMPX_liquidwealth.pdf}
			\includegraphics[scale=0.4]{\figdir/Logs_tranMPX_URE.pdf}
	...
	\begin{figure} 
		\begin{centering}
			\includegraphics[scale=0.4]{\figdir/total_tranMPX_URE.pdf}
...
\begin{figure} 
	\begin{centering}
		\includegraphics[scale=0.55]{\figdir/MPXByDurables_nodurableproxy.pdf}
...
While there is some evidence that our results may be biased upward for those in the top quintiles of liquid assets, this bias will only have a small effect on our overall conclusions. As the relevant number for the monetary policy exercise is the MPX rather than the MPC, we have chosen not to adjust our baseline results using this method and accept that a small bias may exist in our data. It should be noted that such a bias will cause the heterogeneous channels of monetary policy to appear smaller than they actually are.
	...
	\begin{figure} 
		\begin{centering}
			\includegraphics[scale=0.4]{\figdir/MPXBypermURE.pdf}
			\includegraphics[scale=0.4]{\figdir/MPXBypermNNP.pdf}
			\includegraphics[scale=0.4]{\figdir/MPXBypermIncome.pdf}
			\caption{MPX Distribution by URE, NNP, and Income}
	...
	\begin{figure} 
		\begin{centering}
			\includegraphics[scale=0.4]{\figdir/Spouse_tranMPX_liquidwealth.pdf}
	...
	\small
	\bibliographyappendix{AllPapers}
	\normalsize
