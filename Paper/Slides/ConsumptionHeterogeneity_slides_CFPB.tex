...
\frame[t]{
	...
	\only<5->{
	\hspace{-1.17cm}Time Aggregation Bias is large:\\
	Our average MPX $\approx$ 0.5\\
	Blundell, Pistaferri and Preston (2008)  MPX $\approx$ 0.13
	}
}
\frame
{
	\frametitle{Time Aggregation Problem}
		...
		\onslide<1->{
			\begin{tikzpicture}
			\node (img1) {\includegraphics[height=7cm]{../Figures/TimeAgg_simple_1.pdf}};
			\onslide<2->{
				\node (img2) {\includegraphics[height=7cm]{../Figures/TimeAgg_simple.pdf}};
			}
			\end{tikzpicture}
		}
}
