...
\frame
{
	...
	\begin{columns}
		...
		\onslide<1->{
			\begin{tikzpicture}
			\node (img1) {\includegraphics[height=5cm]{../Figures/TimeAggExample1_slides.pdf}};
			\onslide<2->{
				\node (img2) {\includegraphics[height=5cm]{../Figures/TimeAggExample2_slides.pdf}};
			}
			\end{tikzpicture}
		}
		\column{0.5\linewidth}
...
\frame{
	\frametitle{Unhedged Interest Rate Exposure}
	...
	\pause
	\bigskip
	Interest rate change $dR$ $\implies$ Household $i$'s spending changes by:
	\begin{align*}
		dc_i = MPC_i URE_i  \frac{dR}{R}
	\end{align*}
	through the Interest Rate Exposure Channel \textit{alone}.
}
\frame{
...
\frame{
	...
	\item Test Model Microfoundations
