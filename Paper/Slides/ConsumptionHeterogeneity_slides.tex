...
\frame{
	...
	\bigskip
	A 1\% increase in R decreases consumption by 0.33\% due to heterogeneity in interest rate exposure\\
	\bigskip
	This channel is 1 to 6x larger than intertemporal substitution
	\end{columns}
}
...
\frame{
	...
	We calculate
	\begin{itemize}
		\item $\mathcal{E}_R \approx -0.33$
		\item $\mathcal{E}_P \approx -0.66$
		\item $S \approx 0.56$, 1-consumption-weighted MPC
		\item $\sigma$ in the range of 0.1 to 0.5 (maybe)
	\end{itemize}
	$\implies$ the intertemporal substitution channel, $\sigma S \approx 0.05 - 0.28$, is potentially much smaller than the interest rate exposure channel
}
\section{MPC vs MPX}
\frame
{
