...
\date[12/17/2019]{EEA Virtual Congress \\ August 25, 2020 \vspace{2cm} \\ \footnotesize Viewpoints and conclusions stated in this paper are the responsibility of the authors alone and do not necessarily reflect the viewpoints of the Federal Reserve Board or Danmarks Nationalbank.}
\usetheme{Frankfurt}
...
\frame
{
	...
	\bigskip
	\pause
	\textbf{Liquid wealth accurately predicts MPCs across all other dimensions we look at}
}
\frame[t]
{
...
\frame[t]
{
	...
	\vspace{-0.5cm}
	\begin{align*}
	\Delta^N c_i = \alpha^N + \beta^N \Delta^N y_i +\varepsilon_i \qquad \qquad 
	\end{align*}
	\centering
	\begin{tikzpicture}
	\node (img1) {\includegraphics[height=7.5cm,trim= 0 0 0 2cm,clip]{../Figures/basic_regression_complete_slides_level_lincome_head.pdf}};
	\pause
	\node (img2) {\includegraphics[height=7.5cm,trim= 0 0 0 2cm,clip]{../Figures/basic_regression_solow_slides_level_lincome_head.pdf}};
	\pause
	\node (img3) {\includegraphics[height=7.5cm,trim= 0 0 0 2cm,clip]{../Figures/basic_regression_BS_slides_level_lincome_head.pdf}};
	\pause
	\node (img4) {\includegraphics[height=7.5cm,trim= 0 0 0 2cm,clip]{../Figures/basic_regression_slides_level_lincome_head.pdf}};
	\pause
	\node (img5) {\includegraphics[height=7.5cm,trim= 0 0 0 2cm,clip]{../Figures/basic_regression_liquid_wealth_slides_level_lincome_head.pdf}};
	\onslide<1->
	\end{tikzpicture}
}
\begin{comment}
...
\frame[t]
{
	\frametitle{Why Not Blundell, Pistaferri and Preston 2008?}
...
\frame
{
	\frametitle{Time Aggregation Problem in BPP (Crawley 2018)}
	\begin{columns}
		\column{0.5\linewidth}
		\centering
		\onslide<1->{
			\begin{tikzpicture}
			\node (img1) {\includegraphics[height=5cm]{../Figures/TimeAgg_simple_1.pdf}};
			\onslide<2->{
				\node (img2) {\includegraphics[height=5cm]{../Figures/TimeAgg_simple.pdf}};
			}
			\end{tikzpicture}
		}
		\column{0.5\linewidth}
...
\frame
{
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item[] Asset category, mortgage tenure \hspace{1.5cm} \hyperlink{danish_mortgage_market}{\beamerbutton{Danish Mortgage Market}}
		\end{itemize}
...
\frame[t]
{
	...
	\hyperlink{measurement_error}{\beamerbutton{Measurement Error}}
	}
}
\section{Results}
\frame
{
	...
	\begin{columns}
		\column{0.5\linewidth}
		\centering
		\includegraphics[scale=0.4, trim=1cm 0 0 0]{../Figures/MPXByLiquidWealth_level_lincome_head.pdf}
		\column{0.5\linewidth}
		\centering
	\end{columns} 
...
\frame
{
	...
	\begin{columns}
		...
		\column{0.5\linewidth}
		\centering
		\includegraphics[height=7cm,trim=0.8cm 0 8cm 0,clip]{../Figures/basic_regression_liquid_wealth_slides_level_lincome_head.pdf}
	\end{columns}
	\hyperlink{MPXbyNetWealth}{\beamerbutton{MPX by Net Wealth}}
}
\begin{comment}
...
\frame{
	\frametitle{MPX by Unhedged Interest Rate Exposure}
		\begin{center}
		\begin{tikzpicture}
		\node (img2) {\includegraphics[scale=0.53,trim=0 0 0 1.7cm ,clip]{../Figures/MPXByUREdetailsblank_level_lincome_head.pdf}};
		\node (img3)
		{\includegraphics[scale=0.53,trim=0 0 0 1.7cm ,clip]{../Figures/MPXByUREdetails1a_level_lincome_head.pdf}};
		\pause
		\node (img6) {\includegraphics[scale=0.53,trim=0 0 0 1.7cm ,clip]{../Figures/MPXByUREdetailsblank_level_lincome_head.pdf}};
		\node (img7)
		{\includegraphics[scale=0.53,trim=0 0 0 1.7cm ,clip]{../Figures/MPXByUREdetails2a_level_lincome_head.pdf}};
		\pause
		\node (img10) {\includegraphics[scale=0.53,trim=0 0 0 1.7cm ,clip]{../Figures/MPXByUREdetailsblank_level_lincome_head.pdf}};
		\node (img11) {\includegraphics[scale=0.53,trim=0 0 0 1.7cm ,clip]{../Figures/MPXByUREdetails3a_level_lincome_head.pdf}};
		\onslide<1->
		\end{tikzpicture}
	\end{center}
}
...
\frame{
	...
	\draw[blue,thick,->] (aggregatea)  to [in=45,out=225] (aggregateb) node[anchor=north,text = blue] {Through this redistribution channel \textit{alone}};
	\end{tikzpicture}
	\hfill \hyperlink{five_channels}{\beamerbutton{All Five Channels}}
}
\end{comment}
\frame{
	\frametitle{Liquid Wealth Predicts MPX Across Other Dimensions}
	\begin{figure}
	\centering
	\includegraphics[scale=0.3]{\econtexRoot/Figures/NW_predict_from_Liquid.pdf}
	\centering
	\includegraphics[scale=0.3]{\econtexRoot/Figures/URE_predict_from_Liquid.pdf}
	\label{fig:MPXPredict}
	\end{figure}
	Median liquid wealth predicts MPX to approximately four percentage points
}
\section{Conclusion}
\frame{
	...
	\item Liquid Wealth sufficient to determine MPC
...
\frame{
	\frametitle{Danish Mortgage Market}
	\label{danish_mortgage_market}
	\includegraphics[scale=0.6]{../Figures/mortgage_by_maturity.png}
...
\frame{
	...
	\begin{tikzpicture}[remember picture,overlay]
	\draw[red,thick] (2.84,1.07) circle (0.45cm);
	\draw[red,thick] (7.1,0.71) circle (0.45cm);
	\end{tikzpicture} 
	\hyperlink{IRE_channel}{\beamerbutton{Back}}
}
\begin{comment}
\section{Model}
\frame
